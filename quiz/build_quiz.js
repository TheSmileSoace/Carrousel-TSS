...
async function renderOverlay(out, parts){
  ...
  *{margin:0;padding:0;box-sizing:border-box}.st{position:relative;width:1080px;height:1920px;font-family:"Inter",sans-serif}
  ...
  .hook-badge{position:absolute;top:${CONFIG.cornerTagTop||44}px;left:44px;max-width:600px;background:#C24B3A;color:#FFF3EA;font-family:"${FF}";font-weight:700;font-size:27px;letter-spacing:1px;text-transform:uppercase;padding:11px 22px;border-radius:12px;box-shadow:0 8px 22px rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.22);white-space:nowrap}
  ...
  ${parts.question&&CONFIG.difficultyBadge?`<div class="hook-badge">${esc(CONFIG.difficultyBadge)}</div>`:""}
  ...
  ${parts.options&&CONFIG.cornerTag?`<div class="corner-tag">${esc(CONFIG.cornerTag)}</div>`:""}
  ${parts.options?optsBlock:""}
  ${parts.top&&CONFIG.logoPos==="bottom"?`<div class="foot"><img src="${logo}"></div>`:""}</div>`;
